...
# 커스텀 YOLO 모델 로드
model = torch.hub.load('ultralytics/yolov5', 'custom', path=r"C:\Users\이진명\Documents\카카오톡 받은 파일\best.pt", force_reload=True)

# 아두이노 카메라에서 실시간으로 스트리밍하는 영상 받아오기
url = 'http://192.168.35.80:81/stream'  # ESP32 카메라 IP 주소로 변경
...
while True:
    ...
    results = model(frame)
    detections = results.xyxy[0]  # 탐지된 객체의 좌표, 신뢰도, 클래스 정보
